...
public enum SkillsBarLog {
    ...
    public static func logger(_ category: String) -> SkillsBarLogger {
        let logger = Logger(label: "com.skillsbar.\(category)")
        return SkillsBarLogger { level, message, metadata in
...
public struct SkillsBarLogger: Sendable {
    private let logFn: @Sendable (SkillsBarLog.Level, String, [String: String]?) -> Void

    fileprivate init(_ logFn: @escaping @Sendable (SkillsBarLog.Level, String, [String: String]?) -> Void) {
        self.logFn = logFn
    }
